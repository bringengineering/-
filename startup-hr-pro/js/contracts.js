...
`;
  ...
  generatePDF(contractId) {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text('Geun-ro-gye-yak-seo', 105, 28, { align: 'center' });
      ...
      const contractInfo = [
        ...
        `Start Date: ${Utils.formatDate(contract.start_date, 'YYYY.MM.DD')}`,
        contract.end_date ? `End Date: ${Utils.formatDate(contract.end_date, 'YYYY.MM.DD')}` : 'Type: Permanent',
        '',
        `Working Hours: ${contract.weekly_hours} hours/week`,
        `Monthly Salary: ${Utils.formatCurrency(contract.salary)}`,
        `Hourly Wage: ${Utils.formatCurrency(Utils.calculateHourlyWage(contract.salary, contract.weekly_hours))}`,
        '',
        ...
        '',
        '[ Important Notice ]',
        '- Overtime work requires employee consent',
        '- Maximum 52 hours per week (Labor Law)',
        '- Weekend work is voluntary',
        '- Overtime pay: 1.5x regular wage'
      ];

      // 교육비 정보
      if (contract.special_terms?.education_support) {
        const edu = contract.special_terms.education_support;
        contractInfo.push('');
        contractInfo.push('[ Education Support ]');
        contractInfo.push(`- Training: ${edu.name || 'N/A'}`);
        contractInfo.push(`- Amount: ${Utils.formatCurrency(edu.amount || 0)}`);
        contractInfo.push(`- Refund period: ${edu.refund_period || 6} months`);
      }
      ...
      doc.setFont(undefined, 'normal');
      doc.text('Employer: ___________________', 30, yPos);
      doc.text('Employee: ___________________', 120, yPos);
      yPos += 10;
      doc.text(`Date: ${Utils.formatDate(new Date(), 'YYYY.MM.DD')}`, 30, yPos);

      // 푸터
      doc.setFontSize(9);
      doc.setTextColor(100);
      doc.text('Generated by StartupHR Pro', 105, 285, { align: 'center' });
      doc.text(`${Utils.formatDate(new Date(), 'YYYY-MM-DD HH:mm')}`, 105, 290, { align: 'center' });

      // PDF 다운로드
      const filename = `Contract_${employee.name}_${Utils.formatDate(new Date())}.pdf`;
      doc.save(filename);

      Utils.showSuccess('계약서 PDF가 다운로드되었습니다!');

      // 한글 버전도 텍스트로 제공
      this.downloadTextVersion(contractId);
   ...
   * 텍스트 버전 다운로드 (한글 지원)
   */
  downloadTextVersion(contractId) {
    const template = this.generateTemplate(contractId);
    if (!template) return;

    const contract = db.getById('contracts', contractId);
    const employee = db.getById('employees', contract.employee_id);
    const filename = `계약서_한글_${employee.name}_${Utils.formatDate(new Date())}.txt`;

    Utils.downloadFile(template, filename, 'text/plain; charset=utf-8');
    Utils.showSuccess('한글 계약서(텍스트)도 다운로드되었습니다!');
  },
