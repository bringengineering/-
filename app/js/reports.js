...
const Reports = {
  generate() {
    ...
    const teamSat = data.team.members.length > 0
      ? data.team.members.reduce((s, m) => s + m.satisfaction, 0) / data.team.members.length
      : 0;
    ...
    const companyName = Utils.escapeHtml(data.company.name);
    const ceoName = Utils.escapeHtml(data.company.ceo);

    document.getElementById('reportContent').innerHTML = `
      <h1>📊 ${companyName} — 월간 경영 리포트</h1>
      <p style="color:var(--text-light)">보고 기간: ${monthStr} | 생성일: ${now.toLocaleDateString('ko-KR')} | 대표: ${ceoName}</p>
      ...
      <table>
        ...
        <tr>
          ...
          <td>${criticalRisks.length > 0 ? criticalRisks.map(r => Utils.escapeHtml(r.title)).join(', ') : '없음'}</td>
        </tr>
      </table>
      ...
      ${data.financial.govBudgets.map(gb => {
        ...
        return `<p>• ${Utils.escapeHtml(gb.name)}: ${Utils.formatNumber(gb.spent)}만 / ${Utils.formatNumber(gb.totalBudget)}만 (${pct}%)</p>`;
      }).join('')}

      <h2>3. OKR 진행 상황 (${Utils.escapeHtml(data.okrs.currentQuarter)})</h2>
      ${quarter ? quarter.objectives.map((obj, i) => {
        ...
        return `
          <h3 style="font-size:15px;margin:16px 0 8px">O${i+1}: ${Utils.escapeHtml(obj.title)} <span style="color:${Utils.okrScoreColor(avgScore)}">(${avgScore.toFixed(2)})</span></h3>
          <table>
            <tr><th>Key Result</th><th>현재</th><th>목표</th><th>달성률</th></tr>
            ${obj.keyResults.map(kr => {
              const s = Utils.calcOKRScore(kr.current, kr.target, kr.inverse);
              return `<tr><td>${Utils.escapeHtml(kr.title)}</td><td>${kr.current}${Utils.escapeHtml(kr.unit)}</td><td>${kr.target}${Utils.escapeHtml(kr.unit)}</td><td style="color:${Utils.okrScoreColor(s)};font-weight:700">${(s*100).toFixed(0)}%</td></tr>`;
            }).join('')}
          </table>
        `;
      }).join('') : '<p>OKR 데이터 없음</p>'}
      ...
      <table>
        ...
        ${data.projects.map(p => {
          ...
          return `<tr><td>${Utils.escapeHtml(p.name)}</td><td>TRL ${p.currentTRL}</td><td>TRL ${p.targetTRL}</td><td style="font-weight:700">${pct}%</td><td>${debt}건</td></tr>`;
        }).join('')}
      </table>
      ...
      <table>
        ...
        ${data.pipeline.deals.map(d => `<tr><td>${Utils.escapeHtml(d.name)}</td><td>${Utils.escapeHtml(d.product)}</td><td>${Utils.escapeHtml(data.pipeline.stages[d.stage])}</td><td>${Utils.formatNumber(d.amount)}만원</td><td>${Utils.escapeHtml(d.expectedDate)}</td></tr>`).join('')}
      </table>
      ...
      <table>
        ...
        ${data.team.members.map(m => `<tr><td>${Utils.escapeHtml(m.name)}</td><td>${Utils.escapeHtml(m.role)}</td><td>Lv.${m.level}</td><td>${m.satisfaction}/5</td></tr>`).join('')}
      </table>
      ...
      <table>
        ...
        ${data.risks.filter(r => r.status === 'active').sort((a,b) => b.impact*b.probability - a.impact*a.probability).map(r => {
          ...
          return `<tr><td>${Utils.escapeHtml(r.title)}</td><td>${Utils.escapeHtml(r.category)}</td><td style="font-weight:700">${score}</td><td>${grade}</td><td>${Utils.escapeHtml(r.owner)}</td></tr>`;
        }).join('')}
      </table>
      ...
      <p style="font-size:12px;color:var(--text-muted);text-align:center">
        본 리포트는 ${companyName} 경영관리 시스템에 의해 자동 생성되었습니다.<br>
        데이터 기준: ${now.toLocaleDateString('ko-KR')} | 문의: ${ceoName} (대표)
      </p>
    `;
  ...
  _calcOverallHealth(data) {
    ...
    const teamSat = data.team.members.length > 0
      ? data.team.members.reduce((s, m) => s + m.satisfaction, 0) / data.team.members.length
      : 0;
    const techScore = data.projects.length > 0
      ? data.projects.reduce((s, p) => s + p.currentTRL / p.targetTRL, 0) / data.projects.length
      : 0;
